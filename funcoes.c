...
//é mesmo necessário utilizar casting?
...
/*
* *-> acessamos um ponteiro, ''.'' acessamos um elemento do objeto struct...
Pq objeto lista e não ponteiro?
para não perder o primeiro elemento da lista, então usa-se a cópia direta do objeto struct lista...
*/
...
/*
int condicaoCompra(Lista *lista, char  nome[],  float valor, int und){

//RETORNOS:
//1 - VALOR INSUFICIENTE;
//2 - COMPROU X  UNIDADES;
//3 - COMPROU TODAS AS UNIDADES;
    ...
    if(valor >= novo->price){
        if(und < novo->und){
            novo->und -= und;
            retorno = 2;
            printf("\nVoce comprou %d unidades deste livro!!\n", und);
        }else{
            livro *remover = novo;
            lista->start = remover->next;
            free(remover);
            lista->tam--;
            retorno = 3;
            printf("\nVoce comprou todas as unidades deste livro!!\n");
        }
    }else{
        retorno = 1;
